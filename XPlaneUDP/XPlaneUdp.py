...
class XPlaneUdp:
    ...
    def SendCommand(self, cmd):
        msg = b"COMND\x00+"
        cmd  = b"CMND0+sim/operation/load_situation_1"
        cmd = cmd.ljust(500)
        # message = struct.pack("<500s", cmd)
        # print(message)
        message = cmd

        # message = struct.pack("<500s", "CMND0+sim/operation/load_situation_1")
        # message = struct.pack("<5s500s", msg, string)
        self.socket.sendto(message, (self.BeaconData["IP"], self.UDP_PORT))
